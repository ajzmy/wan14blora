...
@torch.amp.autocast("npu", dtype=torch.float32)
...
class WanVideoPipeline(BasePipeline):
    ...
    def __call__(
        ...
        zero_init_steps: Optional[int] = 0,   # for cfg zero star
        # Boundary
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if cfg_scale != 1.0:
                ...
                # cfg zero*
                batch_size = noise_pred_posi.shape[0]
                positive_flat = noise_pred_posi.view(batch_size, -1)  
                negative_flat = noise_pred_nega.view(batch_size, -1)  

                alpha = optimized_scale(positive_flat,negative_flat)
                alpha = alpha.view(batch_size, *([1] * (len(noise_pred_posi.shape) - 1)))
                alpha = alpha.to(noise_pred_posi.dtype)

                if progress_id <= zero_init_steps:
                    noise_pred = noise_pred_posi * 0
                else:
...
def model_fn_wan_video(
    ...
    fuse_vae_embedding_in_latents: bool = True,   # manually set
