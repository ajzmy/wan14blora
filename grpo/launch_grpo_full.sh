...
torchrun \
    ...
    --sft_checkpoint /root/luomingshuang/diffsynth-studio/checkpoints/agibotworld_fun_control_1_3b_text_chunk49_batchsize32_v2/step-18000.safetensors \
    --data_root /root/data/agibot_world_beta_processed_main \
    --reward_dir "${REWARD_DIR}" \
    --reward_mode full \
    --reward_device auto \
    --output_dir /root/xiejunbin/Diffsynth-Studio/grpo/output_full \
    --height 480 \
    --width 640 \
    --num_frames 49 \
    --sampling_steps 20 \
