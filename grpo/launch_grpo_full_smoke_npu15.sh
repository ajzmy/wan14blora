#!/bin/bash
# Full-reward smoke test for GRPO on a single Ascend NPU (device 15).
# Goal: verify the full reward path works end-to-end:
# prepare_conditioning -> sample -> decode -> full reward (PSNR + NDTW + SC) -> backward -> checkpoint.
...
mkdir -p /root/xiejunbin/Diffsynth-Studio/grpo/output_full_smoke_npu15
...
torchrun \
    ...
    --reward_mode full \
    --reward_device auto \
    --output_dir /root/xiejunbin/Diffsynth-Studio/grpo/output_full_smoke_npu15 \
    --height 480 \
    --width 640 \
    --num_frames 49 \
    --sampling_steps 4 \
