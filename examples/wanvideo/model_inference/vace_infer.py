...
from diffsynth.pipelines.wan_vace_v1 import WanVideoPipeline, ModelConfig
...
def prepare_model(args):
    ...
    pipe = WanVideoPipeline.from_pretrained(
        ...
        device="npu",
        model_configs=[
            ModelConfig(path="/root/xiejunbin/Diffsynth-Studio/checkpoints/agibotworld_ti2v_5b/step-40000.safetensors"),
    ...
    pipe.vace = pipe.create_vace_from_dit(pipe.dit, vace_in_dim=48, zero_init_extra=True).to(dtype=pipe.torch_dtype, device=pipe.device)
    # load post-trained weights
    print(f"Loading post-trained checkpoint from: {args.checkpoint}")
    sd = load_state_dict(args.checkpoint)
    m, u = pipe.vace.load_state_dict(sd, strict=False)
    print(f"## Post-trained checkpoint loaded with missing keys: {m}")
    print(f"## Post-trained checkpoint loaded with unexpected keys: {u}")

    print(pipe.vace.get_config())
    return pipe
...
def generate_video(pipe, first_frame_pil, ref_frame_pil, traj_maps, text, T, args):
    ...
    while t < T:
        ...
        vace_video_chunk = traj_maps[t:t + actual]
        ...
        if actual < args.chunk:
            vace_video_chunk = vace_video_chunk + [vace_video_chunk[-1]] * (args.chunk - actual)
